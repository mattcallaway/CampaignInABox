...
def evaluate_system_state(
    ...
    # ── Archive ───────────────────────────────────────────────────────────────
    # Check 1: flat derived/archive/ (where pipeline actually writes)
    archive_root_flat = project_root / "derived" / "archive"
    archive_files_flat = list(archive_root_flat.glob("*")) if archive_root_flat.exists() else []
    # Check 2: state/county subdirectory (future layout)
    archive_root_sub = project_root / "derived" / "archive" / state / county
    archive_dirs_sub = [d for d in archive_root_sub.rglob("*") if d.is_dir()] if archive_root_sub.exists() else []
    # Check 3: latest pipeline run reports archive_built=True
    _archive_from_pipeline = False
    _runs_dir = project_root / "reports" / "pipeline_runs"
    if _runs_dir.exists():
        import json as _json
        _run_jsons = sorted(_runs_dir.rglob("pipeline_summary.json"), key=lambda p: p.stat().st_mtime)
        if _run_jsons:
            try:
                _ps = _json.loads(_run_jsons[-1].read_text(encoding="utf-8"))
                _archive_from_pipeline = bool(_ps.get("archive_built"))
            except Exception:
                pass
    archive_present = bool(archive_files_flat) or bool(archive_dirs_sub) or _archive_from_pipeline
    archive_detail = (
        f"{len(archive_files_flat)} file(s) in derived/archive/"
        if archive_files_flat
        else (f"{len(archive_dirs_sub)} dir(s) in derived/archive/{state}/{county}/"
              if archive_dirs_sub else
              ("pipeline reports archive_built=True" if _archive_from_pipeline else "not found"))
    )
    checks.append(ReadinessCheck(
        name="Archive",
        status=STATUS_PRESENT if archive_present else STATUS_NOT_BUILT,
        detail=archive_detail,
        action=None if archive_present else "Run the pipeline to build the historical archive",
    ))
    if not archive_present:
        actions.append("Run the pipeline -- the DOWNLOAD_HISTORICAL_ELECTIONS step builds derived/archive/")
